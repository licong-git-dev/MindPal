...
#### P3-1. 算法备案（生成式 AI 服务备案）【配套文档 ✅ 已完成 2026-04-23 · 正式提交流程 4-12 周】

**已完成**（代码仓可交付部分）：
- [docs/compliance/README.md](../compliance/README.md) — 合规目录导航 + 核心结论
- [01_BEIAN_OVERVIEW.md](../compliance/01_BEIAN_OVERVIEW.md) —
  三类备案（算法推荐 / 深度合成 / 生成式 AI）的关系 +
  MindPal 当前应做哪两类判断 + 当前形态能否"借"基础模型备案的讨论 +
  推进路径 + 未备案上线风险
- [02_ALGORITHM_SAFETY_SELF_ASSESSMENT.md](../compliance/02_ALGORITHM_SAFETY_SELF_ASSESSMENT.md) —
  完整《算法安全自评估报告》模板（按 TC260-003《生成式人工智能服务安全基本要求》章节）:
  1. 算法基本情况  2. 训练数据安全  3. 语料标注  4. 模型安全评测
  5. 服务过程安全  6. 用户管理  7. 应急处置  8. 透明度
  每章标明 🟩 已有（引用代码）/ 🟨 需填写 / 🟥 缺失（附 9 个 GAP 清单）
- [03_SUPPORTING_MATERIALS_CHECKLIST.md](../compliance/03_SUPPORTING_MATERIALS_CHECKLIST.md) —
  备案提交时的证据材料清单：
  - A 公司资质 / B 产品资料 / C 技术材料 / D 运行应急 四大类 18 项
  - 每项标明来源、准备方法
  - B2 界面截图的 10 张必备清单
  - C4/C5 技术说明文档的建议结构
- [04_FAQ.md](../compliance/04_FAQ.md) — 备案实务 15 个常见问题:
  基础模型借用 / 海外模型可否 / 乙女向风险 / 实名认证 / 对话保留 /
  备案周期 / 边跑边备 / 违规后果 / 监管抽测 / 备案号公示 / 伦理委员会 /
  外部数据 / 备案失败 / 何时要更新 / Timeline 梳理

**未完成**（需要外部动作）：
- 正式向所在地省级网信办提交（需要公司营业执照 + 法定代表人资料）
- 9 个技术 GAP 补齐（见 02 报告末尾"缺口清单"，2-4 周工作量）
- 法律顾问审核服务协议 / 隐私政策 / 未成年人声明

## 六、DEEP_AUDIT_V3 路线最终进度
